...
uses
  ...
  {$ENDIF}
  Classes,
  SysUtils, DateUtils, Windows, idSNTP, untDialogMessage;
...
type
  ...
  public
    ...
    procedure Excecao(Sender: TObject; E: Exception; PAtivaMensagem: Boolean); overload; virtual;
    procedure Excecao(Sender: TObject; E: Exception;
                      PConsultaSQL: String; PAtivaMensagem: Boolean = True); overload; virtual;
  published
...
procedure TCustomExcecao.Excecao(Sender: TObject; E: Exception;
                                 PConsultaSQL: String; PAtivaMensagem: Boolean);
var
  dAno, dMes,
  dDia, dHora,
  dMin, dSeg,
  ...
  try
    ...
    begin
      ...
      if Trim(PConsultaSQL) = EmptyStr then
        ...
        FErroMessage    := E.Message + sLineBreak + 'Consulta SQL: ' + sLineBreak + PConsultaSQL;
      ...
      if FAtivarEmail then
        EMail;
      if FAtivarMensagem and PAtivaMensagem then
    ...
    begin
      ...
      begin
        ShowErrorMessage('Não conectado ao banco de dados.' + sLineBreak + 'Aplicação será finalizada!');
...
procedure TCustomExcecao.Excecao(Sender: TObject; E: Exception; PAtivaMensagem: Boolean);
begin
  Excecao(Sender, E, EmptyStr, PAtivaMensagem);
end;

procedure TCustomExcecao.Excecao(Sender: TObject; E: Exception);
begin
  Excecao(Sender, E, EmptyStr, True);
end;
...
begin
  FDiretorio := untUtil.Diretorio(drLog);
  if Trim(FDiretorio) <> EmptyStr then
  begin
    Result := FDiretorio;
...
begin
  ...
  if Trim(FAppNome) <> EmptyStr then
    Result := iif(Trim(Result) = EmptyStr, Result, Result + sLineBreak) + 'Aplicação: ' + FAppNome;
  if FDataHora > 0 then
    Result := iif(Trim(Result) = EmptyStr, Result, Result + sLineBreak) + 'Data e Hora: ' +
              FormatDateTime(FormatSettings.ShortDateFormat + ' ' + FormatSettings.ShortTimeFormat, FDataHora);
  if Trim(FAppTitulo) <> EmptyStr then
    Result := iif(Trim(Result) = EmptyStr, Result, Result + sLineBreak) + 'Título: ' + FAppTitulo;
  {$IFDEF HAS_FMX}

  {$ELSE}
    if Trim(UsuarioWin) <> EmptyStr then
      Result := iif(Trim(Result) = EmptyStr, Result, Result + sLineBreak) + 'Usuário Windows: ' + UsuarioWin;
  {$ENDIF}
  if Trim(FAppUsuario) <> EmptyStr then
    Result := iif(Trim(Result) = EmptyStr, Result, Result + sLineBreak) + 'Usuário Sistema: ' + FAppUsuario;
  if Trim(FClasseName) <> EmptyStr then
    Result := iif(Trim(Result) = EmptyStr, Result, Result + sLineBreak) + 'Classe Nome: ' + FClasseName;
  if Trim(FComponente) <> EmptyStr then
    Result := iif(Trim(Result) = EmptyStr, Result, Result + sLineBreak) + 'Componente: ' + FComponente;
  if Trim(FFormulario) <> EmptyStr then
    Result := iif(Trim(Result) = EmptyStr, Result, Result + sLineBreak) + 'Formulário: ' + FFormulario;
  if Trim(FTipoObjeto) <> EmptyStr then
    Result := iif(Trim(Result) = EmptyStr, Result, Result + sLineBreak) + 'Tipo Objeto: ' + FTipoObjeto;
  if Trim(FTipoExcecao) <> EmptyStr then
    Result := iif(Trim(Result) = EmptyStr, Result, Result + sLineBreak) + 'Tipo Exceção: ' + FTipoExcecao;
  if Trim(FClasseExcecao) <> EmptyStr then
    Result := iif(Trim(Result) = EmptyStr, Result, Result + sLineBreak) + 'Classe Exceção: ' + FClasseExcecao;
  if Trim(FErroMessage) <> EmptyStr then
    Result := iif(Trim(Result) = EmptyStr, Result, Result + sLineBreak) + 'Erro: ' + FErroMessage;
